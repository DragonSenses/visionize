...
We will then create the `ManageRoles` component. See [Managing Roles in Organizations | Clerk docs](https://clerk.com/docs/organizations/managing-roles).
...
Then add the promotional content of the UpgradeModal inside a new `div`.

feat: Showcase premium features in UpgradeModal

feat: Add promotional content in UpgradeModal
...
**Webhook:** A webhook is a real-time push notification sent to your application as a JSON payload through HTTPS requests.
...
feat: Capture Stripe-Signature from req headers
...
feat: Add webhook for Stripe event verification

Document the development of the webhook endpoint for Stripe event signature verification.
...
feat: Produce checkout session from webhook event
...
Going to make some improves on the documentation, improve error and response handling.

feat: Improve error & response handling in handler

feat: Improve webhook endpoint documentation

- Enhance TSDoc comments for better clarity and detail
- Update return type to NextResponse for consistency with Next.js
- Ensure 200 response is returned to acknowledge receipt of the event
- Improve error handling to prevent timeouts and ensure robust processing
...
This wraps up our handler.

This creates a new [webhook endpoint](https://docs.stripe.com/webhooks/quickstart), a destination on your server that receives requests from Stripe, notifying you about events that happen on your account such as a customer disputing a charge or a successful recurring payment.

It handles requests from Stripe by:

1. Reads the event data in the request body
2. Handles the event
3. Returns a 200 response

Based on the typical routing structure in Next.js, where the file path corresponds to the API endpoint (i.e., `app\api\webook\route.ts`). So the webhook endpoint would be accessible at:

```sh
POST /webhook
```

feat: Implement webhook endpoint handler

Add API POST endpoint at `/webhook` to handle Stripe events and trigger post-payment actions (e.g., sending receipts, fulfilling orders, updating the database).
...
feat: Implement subscription button click handler
